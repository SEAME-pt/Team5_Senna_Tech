...
class YoloSegDecoder:
    def __init__(self, score_threshold=0.25, top_k=200):
    ...
    def _find_output_keys(self, outputs):
        ...
        for name, val in non_proto.items():
            s = val.shape
            if 8400 not in s: continue
            if 4 in s: pre_concat4_bbox = name
            elif 1 in s and max(d for d in s if d != 8400) <= 1: pre_concat4_score = name
            elif 32 in s: pre_concat4_mask = name
    ...
    def decode_to_mask(self, outputs, orig_h, orig_w):
        ...
        if self._output_mode == "pre_concat4":
            ...
            if "det" not in keys: return np.zeros((orig_h, orig_w), dtype=np.uint8)
            det_raw = outputs[keys["det"]]
            det = det_raw.squeeze().astype(np.float32)
            if self.num_channels and det.shape[0] == self.num_channels: det = det.T
            nc = det.shape[1] - 4 - 32
