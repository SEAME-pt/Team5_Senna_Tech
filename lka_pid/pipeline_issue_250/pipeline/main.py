        ...
        with HailoEngine(lane_hef_path, target) as engine_lane, \
            ...
            try:
                while True:
                    ...
                    logging.debug(f"[1. CAMERA] bgr shape: {bgr.shape} dtype: {bgr.dtype}")
                    ...
                    logging.debug(f"[2. INFERENCE] outputs_lane keys: {list(outputs_lane.keys())} outputs_obj keys: {list(outputs_obj.keys())}")
                    ...
                    logging.debug(f"[3. POST-PROCESS] binary_mask shape: {binary_mask.shape} pixels ativos: {np.count_nonzero(binary_mask)}")
                    clean_mask = mask_filters.process(binary_mask)
                    bev_mask = bev.warp(clean_mask)
                    logging.debug(f"[4. BEV] bev_mask shape: {bev_mask.shape} pixels ativos: {np.count_nonzero(bev_mask)}")
                    fit_result = fitter.fit(bev_mask)
                    logging.debug(f"[5. SLIDING WINDOWS] left_fit: {fit_result.left_fit is not None} right_fit: {fit_result.right_fit is not None}")
                    ...
                    logging.debug(f"[6. CTE] cte_norm: {cte:.4f}")
                    pid_return = pid.update(0.0, cte, dt)
                    logging.debug(f"[7. PID] pid_return: {pid_return:.4f}")

                    t_decision = (time.perf_counter() - t0) * 1000
                    if not args.virtual:
                        if abs(last_valid_pid - pid_return) <= 0.4 and not args.virtual:
                            can.send_steering_percent(0x110, pid_return * (-1))
                            logging.debug(f"[8. CAN] steering enviado: {pid_return * (-1):.4f} para 0x110")
