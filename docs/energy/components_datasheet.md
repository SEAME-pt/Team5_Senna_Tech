...
### Techcode TD1484A 5V Regulator

- **Component:** Techcode TD1484A 5V Regulator
- **Voltage (V):** 5 (Output)
- **Max Current (A):** 3.2 (Output Capability)
- **Peak Factor:** 1.0
- **Margin (%):** 20
- **Source/Datasheet:** [Techcode TD1484A Datasheet](https://www.techcode-ic.com/products/td1484a.com)
- **Notes:** This regulator provides the stable 5V supply for the PCA9685 PWM controller and the servo motor. It is a step-down (buck) converter. Its output capability of 3.2A is sufficient to support the servo's stall current of 2.5A, meaning it can handle increased power from an upgraded battery without being a bottleneck for the 5V rail. It appears as CN9R4v.1 on the schematic.
...
- **Notes:** The PiRacer uses two of these motors. They are inductive loads. The stall current (2.3A) is used as the Max Current. A peak factor of 50% (1.5) and a safety margin of 30% are applied as per `issue_76` recommendations. The spreadsheet should account for two units. These motors are driven by the **TB306A2** motor driver (referred to as block TB1 on the schematic). The driver has a continuous current limit of **1.2A per motor**, making it a bottleneck for the motors' full performance.
...
- **Notes:** This servo is an inductive load. The stall current (2.5A) is used as the Max Current. A peak factor of 50% (1.5) and a safety margin of 30% are applied. The PWM signals for this servo are generated by the **PCA9685** controller (appears as PCA1 / PCA2 in the schematic). The PCA9685 is a signal generator and its current consumption is negligible, so it is not a current bottleneck for the servo.
...
- **Notes:** This is the primary power source. While the nominal voltage is 3.6V, it is commonly rated at 3.7V. The Max Current (A) listed is the continuous discharge current of 1.6A (0.5C), with a maximum pulse discharge current of 10.4A (3.25C).
- **Protection (BMS):** The battery pack includes a protection circuit board based on the **S-8254AA IC**. This BMS protects the 3-cell Li-ion pack against overcharge, over-discharge, and overcurrent.
    - **Overcurrent Limit:** The protection limit is determined by the IC and external shunt resistors (0.020Ω), creating a trip threshold estimated at **~7.5A**.
    - **Impact:** This protection is the likely cause of system shutdowns under high load, as it cuts off power when the current draw exceeds its limit.
    - **Datasheet:** [S-8254AA Datasheet](https://www.waveshare.com/w/upload/1/13/S-8254AA.PDF)