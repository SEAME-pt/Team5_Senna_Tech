...
volatile uint8_t flag_mensagem_recebida = 0; // "volatile" It is mandatory for interruptions.
...
int main(void)
{
  ...
  if (MCP2515_Reset() != HAL_OK) {
      printf("ERROR: Reset MCP2515\n");
  }

  if (MCP2515_SetBitrate(CAN_500KBPS, MCP_8MHZ) != HAL_OK) {
      printf("ERROR: Bitrate\n");
  }

  MCP2515_SetNormalMode();

  MCP2515_WriteByte(MCP_CANINTE, 0x03);

  // --- DEBUG CODE (Add this) ---
  uint8_t check_inte = MCP2515_ReadByte(MCP_CANINTE);
  printf("Check CANINTE: Read 0x%02X (Expected: 0x03)\n", check_inte);

  // Clear any old junk on the INT pin before starting
  MCP2515_WriteByte(MCP_CANINTF, 0x00);

  printf("MCP2515 in NORMAL mode and Interrupts ENABLED\n");
  ...
  static uint16_t my_counter = 0; // debug remove later
  while (1)
  {
      ...
      if (HAL_GetTick() - lastSend >= 1000) {
          ...
          my_counter++;//debug remove later

          CAN_Frame frame;
          frame.id = 0x123;
          frame.dlc = 2;
          // Byte Packing Logic (Big Endian / MSB First)
          // Example: If counter = 1000 (0x03E8)
          frame.data[0] = (my_counter >> 8) & 0xFF; // Get the high byte (0x03)
          frame.data[1] = (my_counter) & 0xFF;      // Get the low byte (0xE8)

          if (MCP2515_SendMessage(&frame) == HAL_OK) {
              printf("Sent: ID=0x%03X, Data=%02X %02X\n",
      ...
      //use the flag set in the interrupt to read the message
      /* -------- INTERRUPT-DRIVEN RECEPTION -------- */
      //if (flag_message_received == 1)
      //{
    	  // Lower the flag to avoid re-entering here unintentionally
    	  //flag_message_received = 0;
    	  // Now we read the SPI
    	  //if (MCP2515_ReceiveMessage(&rxFrame) == HAL_OK)
    	  //{
    		  //printf("INT Received ID=0x%03X Data=%02X\n", rxFrame.id, rxFrame.data[0]);
    		  // LED Logic (Remote Command)
    	  ...
    	  // IMPORTANT: If MCP2515_ReceiveMessage fails or if there are more messages,
        // the interrupt will only trigger again if the pin goes up and down.
        // The MCP2515 keeps the pin LOW while there are messages.
        // The ReceiveMessage function already clears the chip's flag, which makes the pin go up (HIGH).
      //}

      /* --------RECEPTION -------- */
      if (MCP2515_ReceiveMessage(&rxFrame) == HAL_OK) {
          printf("Received ID=0x%03X DLC=%d -> ", rxFrame.id, rxFrame.dlc);
...
/* USER CODE BEGIN 4 */
// Esta função é chamada automaticamente pelo STM32 quando o pino INT desce
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    if (GPIO_Pin == MCP_INT_Pin) // Verifica se foi o pino do MCP que chamou
    {
        flag_mensagem_recebida = 1; // Apenas levanta a bandeira!
    }
}
/* USER CODE END 4 */
