...
# ---------------------------------------------------------------
_output_keys  = {}
_output_mode  = None  # "concat4" or "per_scale"
...
def find_output_keys(outputs):
    ...
    # Detect proto: tensor with 32 channels and LARGEST spatial dim (160x160 to 640)
    proto_candidate = None
    proto_spatial   = 0
    for name, val in outputs.items():
        s = val.shape
        if len(s) == 4 and 32 in s:
            spatial = max(d for d in s if d != 1 and d != 32)
            if spatial > proto_spatial:
                proto_spatial   = spatial
                proto_candidate = name
    if proto_candidate:
        _output_keys["proto"] = proto_candidate
        print(f"  -> proto: {proto_candidate} shape={outputs[proto_candidate].shape}")

    # Detect mode: per_scale (10 outputs) or concat4 (4 outputs)
    bbox_scales  = {}
    score_scales = {}
    mask_scales  = {}

    for name, val in outputs.items():
        if name == _output_keys.get("proto"):
            continue
        s = val.shape
        if len(s) != 4:
            continue
        dims = sorted([d for d in s if d != 1])
        # bbox: channel=4
        if 4 in s:
            h = max(d for d in s if d not in (1, 4))
            bbox_scales[h] = name
        # score: channel=1 (but shape has multiple 1s, distinguish by spatial size)
        elif all(d in (1, 20, 40, 80) for d in s):
            h = max(s)
            score_scales[h] = name
        # mask_coef: channel=32
        elif 32 in s:
    ...
    if bbox_scales and score_scales and mask_scales:
        ...
        _output_keys["bbox_scales"]  = bbox_scales   # {80: name, 40: name, 20: name}
        _output_keys["score_scales"] = score_scales
        _output_keys["mask_scales"]  = mask_scales
        NUM_CHANNELS = 4 + 1 + 32  # bbox + score + mask_coef
...
def yolo26_to_mask(outputs, orig_h, orig_w, score_threshold, top_k):
    ...
    Supports:
      ...
      - per_scale mode:  cv2.x + cv3.x + cv4.x per scale + proto
    """
    ...
    if _output_mode == "per_scale":
        ...
        bbox      = concat_scales(keys["bbox_scales"])   # [1,4,8400]
        score     = concat_scales(keys["score_scales"])  # [1,1,8400]
        mask_coef = concat_scales(keys["mask_scales"])   # [1,32,8400]

        det = np.concatenate([bbox, score, mask_coef], axis=1)  # [1,37,8400]
        det = det.squeeze(0).T  # [8400,37]

        scores      = det[:, 4]   # score direto (ja sigmoid pela quantizacao)
        mask_coeffs = det[:, 5:]  # 32 coeficientes
    ...
    if not hasattr(yolo26_to_mask, '_printed_scores'):
        ...
        print(f"[DET] shape={det.shape}  modo={_output_mode}")
        print(f"[SCORES] min={scores.min():.4f}  max={scores.max():.4f}  mean={scores.mean():.4f}")
        print(f"[CHANNELS per channel (0-{det.shape[1]-1})]:")
...
class BEVTransform:
    DEFAULT_SRC = np.float32([
        [0.24, 0.42], [0.76, 0.42],  # top-left, top-right
...
class SlidingWindowsLaneFitter:
    ...
    def _smooth(self, cur, prev, alpha=0.6):
...
def draw_text_overlay(frame, fit: LaneFitResult, inf_ms, fps=None, cpu_ms=None,
                      cpu_user=None, cpu_sys=None, cpu_idle=None):
    """Draws text overlays on the frame already resized for display."""
    h, w = frame.shape[:2]
    result = frame

    def put(txt, pos, color, scale=0.8, thick=2):
        cv2.putText(result, txt, pos, cv2.FONT_HERSHEY_SIMPLEX, scale, (255,255,255), thick+2)
    ...
    # CTE bar
    bm = w // 2
    cv2.line(result, (bm - 100, 58), (bm + 100, 58), (255, 255, 0), 2)
    if fit.cte_pixels is not None:
        dx = int(bm - fit.cte_pixels * 0.2)
        cv2.circle(result, (np.clip(dx, bm-100, bm+100), 58), 7, (255, 255, 0), -1)

    # FPS / Inf
    label = f"Inf: {inf_ms:.1f}ms" + (f"  FPS: {fps:.1f}" if fps else "")
    if cpu_ms is not None:
        label += f"  CPU: {cpu_ms:.1f}ms"
    put(label, (10, h - 15), (0, 0, 0), scale=0.7)

    # CPU sistema
    if cpu_user is not None and cpu_sys is not None and cpu_idle is not None:
        used = cpu_user + cpu_sys
        label2 = f"CPU: {used:.0f}% used  sys:{cpu_sys:.0f}%  free:{cpu_idle:.0f}%"
        put(label2, (10, h - 40), (0, 0, 0), scale=0.65)
...
def process_frame(bgr, outputs, mask_proc, bev, fitter, inf_ms, fps=None):
    ...
    # Update CPU/INF cache every 90 frames
    global _cached_inf_ms, _cached_fps, _cached_cpu_ms, _cached_cpu_u, _cached_cpu_s, _cached_cpu_i
    _debug_counter += 1
    if _debug_counter % 90 == 1:
        _cached_inf_ms = inf_ms
        _cached_fps    = fps if fps else 0.0
        _cached_cpu_ms = ms_cpu_total
        _cached_cpu_u  = cpu_pct_user
        _cached_cpu_s  = cpu_pct_sys
        _cached_cpu_i  = cpu_pct_idle
        ...
        print(f"[SYS] user={cpu_pct_user:.1f}%  sys={cpu_pct_sys:.1f}%  "
              f"idle={cpu_pct_idle:.1f}%  used={cpu_pct_single:.1f}%  (all 4 cores)")
    ...
    return result, fit, _cached_inf_ms, _cached_fps, _cached_cpu_ms, cpu_pct_single, _cached_cpu_u, _cached_cpu_s, _cached_cpu_i
...
def run_image(hef, image_path):
    ...
    result, fit, inf_ms_, _, cpu_ms, cpu_pct, cpu_u, cpu_s, cpu_i = \
    ...
    result = draw_text_overlay(result, fit, inf_ms_, cpu_ms=cpu_ms,
                               cpu_user=cpu_u, cpu_sys=cpu_s, cpu_idle=cpu_i)
            ...
            with ng.activate(ng.create_params()):
                ...
                with InferVStreams(ng, in_p, out_p) as pipeline:
                    ...
                    while True:
                        ...
                        result, fit, inf_ms_, fps_, cpu_ms, cpu_pct, cpu_u, cpu_s, cpu_i = \
